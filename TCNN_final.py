...
class TCNN:
#название такое потому, что пока я зависал в инете, наткнулся на Transiently Chaotic Neural Network
#и на то, как она решает TSP, так что я решил попробовать реализовать ее
...
#print('enter N_RUNS')
N_RUNS = 3                   #кол-во, которые я ее запускаю для получения лучшего результата
#print('enter MAX_IT')
...
kk=" ".join([str(val) for val in op_tour])                
ff = open(file2, 'w')
ff.write(kk)
ff.close